...
public class Util {
	...
	public static X509CRL getCRL() throws InvalidDocumentException {
		...
		try {
			crlData = retrieveByteArrayInputStream();
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			throw new InvalidDocumentException("Cannot create CertificateFactory instance.");
		}
		...
		try {
			...
			throw new InvalidDocumentException("Could not get CRL from data.");
		}
	...
	private static ByteArrayInputStream retrieveByteArrayInputStream() throws IOException {
		return new ByteArrayInputStream(FileUtils.readFileToByteArray(getFileFromResources()));
	}
	
	private static File getFileFromResources() {
		
        String userDir = System.getProperty("user.dir");
		return new File(userDir +"\\src\\main\\resource\\DTCCACrl.crl");
